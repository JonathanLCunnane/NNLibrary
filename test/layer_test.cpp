...
TEST(LayerTest, ReLULayerForwardBackward) {
  CPUContext ctx = CPUContext();
  MockUniformDistribution dist;
  ...
  ReLULayer<CPUContext, 2, 3> layer(ctx, relu_act, dist);
  ...
  std::span<float, 3> output_span = output.get();
  std::array<float, 3> expected_output = {1.5f, 0.0f, 0.0f};
  for (size_t i = 0; i < 3; ++i) {
    EXPECT_EQ(output_span[i], expected_output[i]);
  }
  ...
  std::span<float, 2> grad_x_out_span = grad_x_out.get();
  std::array<float, 2> expected_grad_x_out = {-0.5f, 0.5f};
  for (size_t i = 0; i < 2; ++i) {
    EXPECT_EQ(grad_x_out_span[i], expected_grad_x_out[i]);
  }

  std::span<float, 2 * 3> weights_span = layer.get_weights();
  std::array<float, 2 * 3> expected_updated_weights = {1.0f,  0.5f, -0.5f,
                                                       -1.5f, 1.0f, 1.0f};
  for (size_t i = 0; i < 2 * 3; ++i) {
    EXPECT_EQ(weights_span[i], expected_updated_weights[i]);
  }
  ...
  std::span<float, 1 * 3> biases_span = layer.get_biases();
  std::array<float, 1 * 3> expected_updated_biases = {0.5f, 0.0f, 0.0f};
  for (size_t i = 0; i < 1 * 3; ++i) {
    EXPECT_EQ(biases_span[i], expected_updated_biases[i]);
  }
}